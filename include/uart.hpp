...
public:
    Uart(Config& config, uart_port_t port, Mode mode=MODE_NORMAL): 
        ...
        config(config), port(port), mode(mode)
    {
        ESP_LOGD(TAG, "Init uart%d", port);
...
private:
    ...
    strings split_string(const std::string& str){
        ...
        while (std::getline(ss, word, ':')){
            while(trim.find(word[0]) != std::string::npos) word = word.substr(1);
            while(trim.find(word[word.length()-1]) != std::string::npos) word = word.substr(0, word.length()-2);
            ret.push_back(word);
        }
        return ret;
    }
